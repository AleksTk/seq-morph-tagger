...
class BaseDataBuilder(object):
    ...
    def run(self):
        ...
        vocab_words_train, vocab_tags_train = get_vocab(self.train)
        vocab_words_dev, vocab_tags_dev = get_vocab(self.dev)
        vocab_words_test, vocab_tags_test = get_vocab(self.test)
...
def get_singletons(dataset):
    counter = Counter(word for words, tags in dataset for word in words)
...
class BaseCoNLLDataset(object):
    ...
    def __init__(self, filename, processing_word=None, processing_tag=None, max_iter=None, shuffle=False,
                 use_buckets=False, batch_size=None, sort=False):
        """
        Args:
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            for words, tags in sentences:
                yield words, tags
                n += 1
            print("n=%d, snt-num=%d" % (n, len(sentences)))
        elif self.sort:
            ...
            for bucket in bucket_list:
                ...
                for words, tags in sentences[offset: offset + self.batch_size]:
                    yield words, tags
                    n += 1
...
def get_vocab(dataset):
    ...
    Returns:
        a set of all the words and tags in the dataset
